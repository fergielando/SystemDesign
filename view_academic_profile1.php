...
if (isset($_GET['UID'])) {
    ...
    // Fetch Student Data
$query = "SELECT u.*, s.UnderGradStudentType FROM undergradstudent s
RIGHT JOIN user u ON u.UID = s.StudentID
WHERE u.UID = '$uid'";
...
// Check if the key exists and assign a default value if not
$user['UnderGradStudentType'] = isset($user['UnderGradStudentType']) ? $user['UnderGradStudentType'] : '';

// Re-fetch user data
$query = "SELECT * FROM user WHERE UID = '$uid'";
$result = mysqli_query($conn, $query);
if (mysqli_num_rows($result) > 0) {
// Merge data from the user table
$user += mysqli_fetch_assoc($result);
}
}


// Now $user should contain the combined data from both user and undergradstudent tables
...
<div class="academic-profile-container">
   ...
   <p><strong>Student Status:</strong> <?php echo isset($user['UnderGradStudentType']) ? $user['UnderGradStudentType'] : 'N/A'; ?></p>
</div>

<form action="" method="post">
    <label for="student_status">Student Status:</label>
    <select name="student_status" id="student_status">
        <option value="full-time" <?php echo ($user['UnderGradStudentType'] == 'Full-Time') ? 'selected' : 'Full-Time'; ?>>Full-Time</option>
        <option value="part-time" <?php echo ($user['UnderGradStudentType'] == 'Part-Time') ? 'selected' : 'Part-Time'; ?>>Part-Time</option>
    </select>
