...
<script>
    // Get references to the select element and faculty table
    const deptSelect = document.getElementById('dept_id');
    const facultyTable = document.getElementById('faculty-table');

    // Event listener to update the faculty table when a department is selected
    deptSelect.addEventListener('change', function () {
        const selectedDeptID = deptSelect.value;

        // Fetch faculty members using AJAX
        fetch('getfac.php?dept_id=' + selectedDeptID)
            .then(response => response.json())
            .then(data => {
                // Clear existing rows in the faculty table
                facultyTable.querySelector('tbody').innerHTML = '';

                // Populate the faculty table with the retrieved data
                data.forEach(faculty => {
                    const row = facultyTable.querySelector('tbody').insertRow();
                    const cell1 = row.insertCell(0); // Faculty ID
                    const cell2 = row.insertCell(1); // Percent Time
                    const cell3 = row.insertCell(2); // Faculty Name
                    const cell4 = row.insertCell(3); // Total Classes PT
                    const cell5 = row.insertCell(4); // Total Classes FT

                    cell1.textContent = faculty.FacultyID;
                    cell2.textContent = faculty.PercentTime;
                    cell3.textContent = faculty.FirstName + ' ' + faculty.LastName; // Display Faculty Name
                    cell4.textContent = faculty.TotalClassesPT; // Display Total Classes Part-Time
                    cell5.textContent = faculty.TotalClassesFT; // Display Total Classes Full-Time
                });
            })
            .catch(error => {
...
courseSelect.addEventListener('change', function() {
    ...
    // Fetch faculty members for the selected course's department
    fetch(`getfacultyforcourse.php?course_id=${selectedCourseID}`)
        .then(response => response.json())
        .then(data => {
            // Clear existing options in the faculty select
            facultySelect.innerHTML = '';

            // Populate the faculty select with the retrieved data
            data.forEach(faculty => {
